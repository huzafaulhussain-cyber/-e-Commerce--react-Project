...
const Product = () => {
    ...
    const [filteredProducts, setFilteredProducts] = useState(mens_kurta);
    ...
    useEffect(() => {
        ...
        let products = [...mens_kurta];

        if (colorValue) {
            const colors = colorValue.split(',');
            products = products.filter(p => colors.includes(p.color.toLowerCase()));
        }

        if (sizeValue) {
            const sizes = sizeValue.split(',');
            products = products.filter(p => p.size.some(s => sizes.includes(s.name)));
        }

        if (priceValue) {
            const [min, max] = priceValue.split('-').map(Number);
            products = products.filter(p => p.discountedPrice >= min && p.discountedPrice <= max);
        }

        if (discount) {
            const [min] = discount.split('-').map(Number);
            products = products.filter(p => p.discountPersent >= min);
        }
    ...
    const handleFilter = (value, sectionId) => {
        ...
        if (filterValue.length > 0 && filterValue[0].split(",").includes(value)) {
            ...
            // For price, allow only one selection
            if (sectionId === 'price') {
